# Nalin Ratnaike

**Type:** Out-of-state / multi-property investor owner
**Status:** Active
**Portfolio size:** 6 properties
**Relationship start:** [add date]
**Preferred contact:** [email / phone — add]
**Email:** [add]
**Phone:** [add]
**Mailing address:** [add]

---

## Properties managed

- [[421-ridgeview]]
- [[602-carver]]
- [[192-diamond-ranch]]
- [[1707-aleia-cove]]
- [[5401-arroyo-trl]]
- [[5217-ambergate-ln]]

---

## Owner preferences & notes

- Multi-property investor — prefers consolidated reporting across all 6 properties
- [add communication cadence preference — monthly, quarterly, ad-hoc?]
- [add approval thresholds — does he want to approve repairs over $X?]
- [add preferred vendors if any]

---

## Active items

- 602 Carver: All In One Foundation report on file — Drive file ID `1OoTqPBLCJ8SpwWcpYeYHC121fPNYxemO`
- 192 Diamond Ranch: property folder — Drive file ID `1zo-vVPtRz2oesy4q_Dw6i9_Hg6jemjFq`; Wise Water Solutions well estimate — Drive file ID `1bZN9HBJPz9-eaJvahBk_H0Kn7pqQjHaW`
- Property update and approval form in progress (spans all 6 properties)

---

## Communication history

_Log significant calls, emails, decisions here. Most recent on top._

- [date] — [summary of interaction, key decisions, follow-ups]

---

## Open questions / to-do

- [ ] Confirm preferred contact method and cadence
- [ ] Document approval thresholds for repairs/expenses
- [ ] Confirm mailing address for statements

---

## Tags

#owner #active #multi-property #investor


---

## Legal / Dispute

- [[Nalin Ratnaike - Termination Dispute]] — termination settled (eff. July 1, 2026); financials open under Paragraph 17; documented pattern of three altered termination documents (June 19–22, 2026), including an altered signing date on "Termination 1907.pdf."
